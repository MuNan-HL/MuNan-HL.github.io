...
> 4. 如何搭建 Hexo 个人博客?
```
- [参考资料 Hexo](https://hexo.io/zh-cn/docs/)
...
``` md
> 1. 什么是图床?
- 图床: 即图像托管服务器, 用户可以将图片上传到图床服务器，服务器会返回一个可供在网站上使用的图片链接。
...
- 2. 使用 npx 命令， 运行 hexo 模块相关的命令.

- 3. 当办法一不生效时, 在我的电脑 - 属性 - 高级系统设置 - 环境变量 - 系统环境变量下 - 新增 HEXO_HOME 变量
(变量值为 hexo 模块所在的文件路径) - 然后在 系统变量 Path 中新增 %HEXO_HOME% 变量 - 确认.
...
- 1. Everything up-to-date, 表示一切都更新到最新状态, 即本地 git 仓库未有修改, 因此可能是没有进行 add 或 commit, 
因此重新执行以下代码即可:
```
``` git
git add .
...
- 补充知识1: npx 解决的主要问题，就是通过调用项目内部已安装的模块, 解决安装了相关模块的命令由于未配置环境变量
而导致的命令无法运行的问题”，npx 运行的时候，会到 node_modules/.bin 路径和环境变量 $PATH 里面，检查命令是否
存在，所以系统命令也可以调用，即上面的命令无法运行的时候加上 npx 的话也许就可以成功了.
```
