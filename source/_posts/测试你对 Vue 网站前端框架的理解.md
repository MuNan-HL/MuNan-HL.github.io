...
``` md
> 2. 你知道 key 键的作用吗？
- 1. 在 Vue Web 网站前端框架中, key 键的作用是为了维护 VDOM 虚拟文档对象模型的渲染和更新, 保证其性能和准确性. 通过 key 键唯一标识元素, 如果 key 键没有改变, 则会复用该元素, 反之则会新增一个元素. 减少渲染次数，提高性能.
```
...
- 1. SPA 单页面应用, 即 CSR 客户端渲染, 优点是单次请求, 交互性好, 缺点是 SEO 支持不够好, 且首屏加载时间长.
```

``` md
> 68. mvc 模型-视图-控制器模式和 mvvm 模型-视图-视图模型模式的理解
- 1. MVC model 模型- view 视图- controller 控制器是指, 通过 view 视图来和用户交互, model 模型存储数据, controller 控制器处理数据并通知 view 视图进行更新.

- 2. MVVM model 模型- view 视图- viewModel 视图模型是指, 通过 view 视图和用户交互, model 模型存储数据, viewModel 视图模型处理 model 模型并通知 view 视图. 
...
- 1. 因为 Vue2 中的响应式数据是通过 defineProperty 定义属性实现的响应式. 你直接给对象新增属性, 就会导致该属性并非响应式的属性, 因此页面无法响应. 在 Vue3 中解决了这个问题.
```
...
- 1. 因为当 v-if 条件渲染和 v-for 列表渲染同时使用时，Vue 网站前端框架会为每个循环的元素创建一个独立的渲染函数实例。这意味着即使某些元素不满足 v-if 的条件而被跳过，它们的渲染函数仍然会被创建并执行。`这会导致不必要的计算和内存消耗，尤其是在处理大量数据时。`
```
...
- 1. key 键的作用就是唯一标识元素, 为了维护 VDOM 虚拟文档对象模型元素的更新和渲染, 在 diff 比较差异的过程中, 如果 key 键不变, 则会复用元素, 否则会重新渲染元素.
```
...
- 补充知识1, 通过异步组件，`可以减少初始加载时的文件体积，提高应用的性能`只有在需要使用某个组件时才会加载对应的代码，避免了不必要的网络请求和资源加载
```
...
- 1. 简单来说就是 5 个点吧, 第一点是 template 模板编译, 第二点是响应式系统, 第三点是 VDOM 虚拟文档对象模型, 第四点是组件的生命周期, 第五点是内置指令.
```
...
``` md
> 92. Vue Web 前端框架中的 nextTick 下一个更新周期的原理.
```
