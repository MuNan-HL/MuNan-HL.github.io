...
``` md
> 38. 你怎么理解函数?
- 1. 在数学概念中, 函数就是指一种映射关系, 由集合 A 到集合 B 的映射关系. 映射关系可以是一对一, 也可以是一对多. 而 JS 语言中的函数，本质就是数学概念中函数的具象化.
```

``` md
> 39. 你怎么理解编程范式这一概念?
- 1. `编程范式就是一种编程风格`. 开发人员通过遵循一系列特定的概念, 原则和实践, 它塑造了程序员使用编程语言解决问题的方式.
```
...
``` md
> 45. 什么是闭包?
- 1. 闭包就是内部函数. `闭包有两个特点`, 首先是它可以访问外部函数的变量, 其次是它可以记住它的上下文.
```
...
``` md
> 47. 在 ES6 第六版 JS 语言规范中如何一个函数如何接受可变参数?
...
``` md
> 52. 函数式编程的原则
- 1. small, smaller. 第一个原则是要小, 第二个原则是要更小.
```

``` md
> 53. 你怎么理解 API 应用程序接口?
- 1. API 应用程序接口, 就是指内部和外部进行通信式所制定的一组规则. 
```

``` md
> 54. 数据结构中的数组和列表的区别是什么？
- 1. 数组, 线性的, 连续的存储空间, 而列表, 线性的, 离散的存储空间.
