...
- 1. `导入分为默认导入和命名导入`, `导出同理, 分为默认导出和命名导出`, 需要注意的是, `默认导出一个模块只能存在一个`. 同时, 开发时建议使用命名导出, 会使得模块结构更加清晰.
```
- [参考资料 到如何导出](https://zh.javascript.info/import-export)

``` md
> 48. 动态导入是什么?
- 1. 区别于静态导入(47 题的导入为静态导入)一种导入方式, `其目的是一种让开发者可以根据条件或者在运行时进行导入而设计的一种导入方式`. 
```
